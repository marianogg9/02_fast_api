...
def test_get_all(client):
    ...
    response = client.get(
        "/all", headers={"Authorization": login(client).json["auth_token"]}
    )
    assert response.status_code == 200
...
def test_get_anime(client):
    response = client.get(
        ...
        headers={"Authorization": login(client).json["auth_token"]},
    )
    assert response.status_code == 200
...
def test_get_profile(client):
    response = client.get(
        "/profile", headers={"Authorization": login(client).json["auth_token"]}
    )
...
def test_logout(client):
    login(client)
    response = client.post(
        "/logout", headers={"Authorization": login(client).json["auth_token"]}
    )
    assert response.status_code == 200
